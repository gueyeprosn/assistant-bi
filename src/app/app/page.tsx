...
export default async function TodayPage() {
  ...
  const [todayAppts, handoffs, monthCount, pendingPay, inboundToday] = await Promise.all([
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-3xl font-bold text-navy">{t(lang, "today")}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <Stat n={handoffs} label={t(lang, "missed")} href="/app/messages" />
      </div>
      ...
      <section className="card overflow-hidden">
        <div className="px-4 py-3 border-b border-line flex justify-between items-center">
          ...
          <Link href="/app/calendrier" className="font-semibold text-navy min-h-11 inline-flex items-center">
            {t(lang, "seeAll")}
          </Link>
        </div>
        {todayAppts.length === 0 ? (
